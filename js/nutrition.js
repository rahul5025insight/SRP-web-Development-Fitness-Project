...
function set_ratio(c, p, f) {
    // Grab them once
    const carb = document.getElementById('carbratio');
    const prot = document.getElementById('proteinratio');
    const fat = document.getElementById('fatratio');

    // Only set values if the inputs are found
    if (carb && prot && fat) {
        carb.value = c;
        prot.value = p;
        fat.value = f;
...
function update_ratio() {
    const carbInput = document.getElementById("carbratio"),
        protInput = document.getElementById("proteinratio"),
        fatInput = document.getElementById("fatratio");

    if (carbInput && protInput && fatInput) {
        // Parse numeric ratios (fallback to 0 if empty or invalid)
        const cratio = parseFloat(carbInput.value) || 0;
        const pratio = parseFloat(protInput.value) || 0;
        // Compute and clamp to [0,100]
        fatInput.value = Math.max(0, Math.min(100, 100 - cratio - pratio));
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Set default macro ratios once inputs are in the DOM
    set_ratio(40, 30, 30);

    // Recalculate whenever the user clicks or types anywhere
